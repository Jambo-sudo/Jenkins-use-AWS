# Jenkins in AWS
We take the [Historical-document-layout-analysis](https://github.com/Jambo-sudo/Historical-document-layout-analysis) as an example to show how to deploy project to AWS EC2 instance through jenkins. You can fork this porject in your GitHub and follow this tutorial. We will create 2 EC2 instances, one for management and the other for running our project. The project is triggered by GitHub webhook. When the GitHub is pushed any commit, jenkins will automatically build the project.

## Requirement
In order to complete the deployment, you need an AWS account and some basic knowledge about AWS. We will create two EC2 instances, one is jenkins manager and another is jenkins slave. Our project will running in jenkins slave. The minimum requirements for these two instances are as follows:
...
We use Ubuntu 20.04 instead of the same operating system as jenkins manager (Amazon Linux). Because Amazon Linux has encountered some problems in docker, if you use different project, than you should choose OS system based on that.  
...
Remote root directory is the root directory of jenkins slave instance. You can find it in the EC2 console. Different OS will have different root directories or you can simply get this directory by `pwd`.
...
3. Branches to build. Use 'main' or just leave it empty because we only have one branch. (Note: there may multiple branches in your project.)
